...
const webSecrets = await jiti.import("./web-secrets.ts");
...
test("native module load failures are deferred until creation and include repair instructions", async () => {
  ...
  runInNewContext(outputText, { exports, process, require(id) {
    ...
    if (id === "./web-secrets") return webSecrets;
    return require(id);
  } });
...
test("shell environment defaults to UTF-8 locale when the host has none", async () => {
  ...
  const envWithoutLocale = Object.fromEntries(
    Object.entries({ ...process.env, PI_WEB_PASSWORD: "secret" })
      .filter(([key]) => !["LANG", "LC_ALL", "LC_CTYPE"].includes(key)),
  );
  const exports = {};
